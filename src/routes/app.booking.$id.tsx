...
import { getBooking, cancelBooking, type Booking } from "@/lib/app/bookings";

export const Route = createFileRoute("/app/booking/$id")({ component: BookingPage });

const STEPS: { key: Booking["status"]; label: string }[] = [
...
function BookingPage() {
  ...
  const [booking, setBooking] = useState<Booking | undefined>(() => getBooking(id));

  useEffect(() => { setBooking(getBooking(id)); }, [id]);
  ...
  const doCancel = () => {
    if (!confirm("Cancel this booking? You won't be charged.")) return;
      ...
      <div className="mx-auto max-w-2xl space-y-4">
        <div className="rounded-2xl border border-border bg-card p-6 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {cancelled ? "No charges applied." : `${booking.workerName} will arrive at ${booking.slot}`}
          </p>
          <p className="mt-2 text-xs text-muted-foreground">Booking ID · <span className="font-mono">{booking.id}</span></p>
        </div>
        ...
        <div className="rounded-2xl border border-border bg-card p-5">
          ...
          <div className="space-y-2.5 text-sm">
            <Row icon={<Calendar className="h-4 w-4" />} k="Service" v={`${booking.service} · ${booking.duration} hr${booking.duration > 1 ? "s" : ""}`} />
            <Row icon={<Clock className="h-4 w-4" />} k="Time slot" v={booking.slot} />
            <Row icon={<MapPin className="h-4 w-4" />} k="Address" v={booking.address} />
            {booking.notes && <Row icon={<MessageCircle className="h-4 w-4" />} k="Notes" v={booking.notes} />}
            <Row icon={<Shield className="h-4 w-4" />} k="Payment" v={`${booking.payment.toUpperCase()} · ₹${booking.total}`} />
          </div>
        </div>
          ...
          <div className="rounded-2xl border border-border bg-card p-5">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <div className="font-semibold">{booking.workerName}</div>
                <Link to="/app/worker/$id" params={{ id: booking.workerId }} className="text-xs" style={{ color: "var(--teal)" }}>View profile</Link>
              </div>
              <button className="flex h-9 w-9 items-center justify-center rounded-full border border-border"><Phone className="h-4 w-4" /></button>
              <button className="flex h-9 w-9 items-center justify-center rounded-full border border-border"><MessageCircle className="h-4 w-4" /></button>
            </div>
          </div>
        )}
        ...
        <div className="flex gap-3">
          ...
          {!cancelled && booking.status !== "completed" ? (
            <button onClick={doCancel} className="flex-1 rounded-xl py-3 text-sm font-semibold"
              style={{ background: "transparent", border: "1px solid var(--coral, #ff7a7a)", color: "var(--coral, #ff7a7a)" }}>
              Cancel booking
            </button>
          ) : (
